...
class App_themesmanager_migration
{
    /**
    ...
    /**
     * Path to theme migration classes
     *
     * @var string
     */
    ...
    /**
     * Current theme migration version
     *
     * @var mixed
     */
    ...
    public function __construct($theme = null)
    {
        ...
        $this->_migration_table = db_prefix().'themes';
        // Loaded via the autoload feature, not theme name in this case is passed, ignore
        if ($theme === null) {
            return;
        }
        ...
        $this->_migration_path = APP_THEMESMANAGER_PATH . $this->theme_name . '/' . 'migrations/';

        // Set the current theme version
    ...
    {
        $migrations = [];
        //var_dump(glob($this->_migration_path . '*_*.php'));
    ...
    {
        $row = $this->ci->db->select('installed_version')->where('theme_name', $this->theme_name)->get($this->_migration_table)->row();
     ...
     * Set the current theme version
     */
    protected function set_current_theme_version()
    {
        $theme = $this->ci->app_themesmanager->get($this->theme_name);

        $this->_migration_version = str_replace('.', '', $theme['headers']['version']);
    }
    ...
    {
        $this->ci->db->where('theme_name', $this->theme_name);
